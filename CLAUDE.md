...
- `npm run signals:collect [-- --profile <name>]` — pull candidate feeds into `data/_candidates[-<name>].json`. **Which** sources are collected lives in `config/sources/<name>.json`, not in the script. Profiles are standalone — no inheritance — because the generic run collects the shared sources weekly anyway and the ledger dedupes them, so an inherited half would be empty by construction. The output path is derived from the profile name, so a sector run cannot overwrite the generic pool, and an unknown profile exits 1 rather than falling back.
...
- `npm run sources:discover` / `npm run sources:promote` — the source nomination loop. A finder run nominates a practitioner or publication it drew a signal from into `data/source-nominations/`; `discover` resolves it to a verified feed (the finder never guesses a feed URL); you move the file into `accepted/` or `rejected/`; `promote` appends accepted ones to their `config/sources/<profile>.json`. Rejections stay on disk as the memory that stops re-nomination. Feeds only — a feed is verifiable by code, a search term is not.
...
- **Rationale goes in the draft, as a `_review` block** (`under`, `note`, `reviewer`) added before the move. The folder is the decision; `_review` carries only the reason, so the two cannot disagree. `reviewer` defaults to `git config user.name`. `promote` strips `_review` and `$schema` before publishing — editorial commentary about a named source must never reach the live site — and appends one event per decision, human and finder alike, to `data/_review-log.jsonl`. That file is **gitignored**: free-text judgment about named third parties does not belong in a public history. A draft moved with no `_review` is recorded as `unrecorded` and counted, never refused.
- The rejection vocabulary lives in `scripts/lib/review-schema.mjs`. `schemas/signal-draft.schema.json` is generated from it by `npm run schema:build` and gives editors enum autocomplete via each draft's inline `$schema` key; a test fails if the two drift.
...
scripts/
  lib/              # content loader, phenomenon-schema mirror, derive.mjs, source-profile.mjs
  __tests__/        # node --test suite for the scripts above
config/
  sources/          # candidate-collector source profiles, one JSON per run kind
```
...
- `sourceUrl` is optional, but when present must be an absolute `http`/`https` URL whose host is not a reserved placeholder (`example.com/.net/.org` and their subdomains, `localhost`, and the `.test`/`.example`/`.invalid`/`.localhost` TLDs). Three signals reached the live site with `https://example.com/...` and invented figures attributed to VTT and MIT Technology Review; nothing in the schema had looked at `sourceUrl` at all. They were unpublished on 2026-08-12 and the check added. The ledger still records them as `published`, so no future run re-surfaces them.
