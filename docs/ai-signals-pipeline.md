...
| `scripts/collect-candidates.mjs` | Feed collector; **which** sources it pulls comes from a profile | yes |
| `config/sources/<profile>.json` | The sources one run collects. Standalone, no inheritance | **yes — edit these, not the script** |
| `scripts/lib/source-profile.mjs` | Profile loading, validation and output-path derivation | yes |
| `scripts/validate-signals.mjs` | Schema validator; runs first in `npm run build` | yes |
| `scripts/lib/signal-schema.mjs` | The signal schema, shared by validate and promote | yes |
| `scripts/lib/review-schema.mjs` | The rejection vocabulary and the review-event shape | yes |
| `schemas/signal-draft.schema.json` | Editor autocomplete for drafts; generated by `npm run schema:build` | yes (generated) |
| `scripts/promote-signals.mjs` | Publishes reviewed drafts; the only schema gate | yes |
| `scripts/sources-discover.mjs` | Resolves a nominated source to a verified feed | yes |
| `scripts/sources-promote.mjs` | Appends accepted nominations to their profile | yes |
| `data/_seen-ledger.jsonl` | Append-only memory of every item seen | **yes — this is state** |
| `data/_candidates*.json` | Per-run candidate pool for the prompt, one per profile | no (gitignored, regenerated) |
...
| `data/_review-log.jsonl` | Why each decision was made, human and finder alike | no (gitignored — names third parties) |
| `data/source-nominations/` | Nominated sources awaiting review, and the decided folders | no (gitignored — names individuals) |

The three gitignored stores at the bottom carry judgment about named people and
organisations, which is why none of them is committed on a public repo — and why
each is local to one machine. That constraint is the same one an eventual
private store would remove.
...
npm run signals:collect      # 2. collect the 'generic' profile -> data/_candidates.json
#                                 (a sector run adds --profile <dim> if it has one)
...
#                                 one per signal, appends to data/_finder-rejected.jsonl,
#                                 and may nominate sources into data/source-nominations/
#                              4. review data/signal-drafts/: add a "_review" block
#                                 saying why, then mv each file into accepted/ or rejected/
npm run signals:promote      # 5. validate, publish, record every decision in the ledger
#                                 AND every rationale in data/_review-log.jsonl

# optional, after a run that nominated sources:
npm run sources:discover     # 6. resolve each nominee to a verified feed
#                              7. review data/source-nominations/, mv to accepted/ or rejected/
npm run sources:promote      # 8. append accepted ones to their config/sources/<profile>.json
```
